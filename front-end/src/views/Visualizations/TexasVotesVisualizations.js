...
const PoliticiansChart = () => {
	...
	return (
		<>
			<Title level={3}>Politician Fundraising</Title>
			<div className={styles.chart}>
				<BubbleChart
					...
					data={data}
					bubbleClickFun={(val) => {
						history.push(`/politicians/view/${polID.get(val)}`)
					}}
				/>
			</div>
		</>
...
const DistrictsChart = () => {
	...
	return (
		<>
			<Title level={3}>Demographics of Congressional Districts</Title>
...
const ElectionsChart = () => {
	...
	useEffect(() => {
		const parseData = (data) => {
			const electionPartyResults = data
				.filter((e) => e.results)
				.map((e) => {
					const { results, id } = e
					...
					output.name = `${year.getFullYear()} General US House ${
						e.district.number
					}`
					return output
				})
				.filter((r) => r)
				.sort((a, b) => b.R / b.total - a.R / a.total)
...
export { PoliticiansChart, DistrictsChart, ElectionsChart }
