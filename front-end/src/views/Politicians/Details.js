...
export default function Details() {
	...
	if (loaded) {
		if (contact.social_media) {
			contact.social_media.map(
				(social) => (socials[social.type] = social.id)
			)
		}
				...
				<div className={styles.politicianDescription}>
					...
					<article className={styles.politicianDetails}>
						<div className={styles.politicianGeneralInfo}>
							<div>
								<Text strong>Party: </Text>
								<Text>{party_mappings[party]}</Text>
							</div>
							{district ? (
								<div>
									<Text strong>District: </Text>
									<Link to={`/districts/view/${district.id}`}>
										{district.number === -1
											? "Texas"
											: districtName(
													district.type,
							...
							{current ? (
								<Fragment>
									<div>
										<Text strong>Current Office: </Text>
										<Text>{`${officeName(current)}`}</Text>
									</div>
								</Fragment>
							) : null}
						</div>
						...
						<div className={styles.politicianSocials}>
							<div>
								...
								>
									<GlobalOutlined /> Campaign Website
								</a>
							</div>
							<div>
								<a
					...
					{Object.keys(socials).includes("twitter") ? (
						...
						<Paragraph>
							{name} does not have Twitter or Facebook.
						</Paragraph>
					)}
						...
						{district ? (
							<div>
								<Paragraph>
									...
									which spans {district.counties.length} count
									{district.counties.length === 1
										? "y"
										: "ies"}
									. Here are the count {name} would represent:{" "}
								</Paragraph>
								<br />
								<List
							...
							<Paragraph>
								There is no district information for {name}
							</Paragraph>
						)}
					</article>
					<Divider />
					...
					<article className={styles.electionDetails}>
						...
						{fundraising ? (
							<section className={styles.electionSection}>
								<Title level={4}>
									2019-2020 Fundraising Information
								</Title>
								<Paragraph>
									{name} has raised{" "}
									{formatAsMoney(fundraising.raised)} for
									their current campaign.
								</Paragraph>
								<Paragraph>
									{name} has spent{" "}
									{formatAsMoney(fundraising.spent)} on their
									current campaign.
								</Paragraph>
								<Paragraph>
									{name} currently has{" "}
									{formatAsMoney(fundraising.remaining_cash)}{" "}
									on hand.
								</Paragraph>
								<List
									header={
										<Text
											...
											style={{ fontSize: "18px" }}
										>
											{name}'s Contribution by Industry
										</Text>
									}
									bordered
							...
							<section>
								...
								<Paragraph>
									{name} doesn't have any fundraising
									information on file.
								</Paragraph>
							</section>
						)}
						<section className={styles.electionSection}>
							...
							{elections ? (
								<Fragment>
									<Paragraph>
										{name} is in the following elections:
									</Paragraph>
								...
								<Paragraph>
									{name} is not up for re-election.
								</Paragraph>
							)}
						</section>
					</article>
